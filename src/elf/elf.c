...
swaddr_t present_func(swaddr_t addr) {
	int i;
	for(i = 0; i < nr_symtab_entry ; i++) {
...
void print_now_func(swaddr_t addr, int index) {
	int i;
	for(i = 0; i < nr_symtab_entry ; i++) {
		if((symtab[i].st_info & 0xf) == STT_FUNC)
			if(addr >= symtab[i].st_value && addr <= (symtab[i].st_value+symtab[i].st_size))
				printf("#%d 0x%x in %s\n",index,addr,strtab+symtab[i].st_name);
	}
}
