...
bool concat(or_flags_, SUFFIX)(DATA_TYPE_S a,DATA_TYPE_S b) {
...
make_helper(concat(or_ib2rm_, SUFFIX)) {
	...
	if(m.mod == 3) {
		...
		concat(or_flags_, SUFFIX)( REG(m.R_M), imm);
		REG(m.R_M) |= imm;
		...
		concat(or_flags_, SUFFIX)( MEM_R(addr), imm);
...
make_helper(concat(or_i2ra_, SUFFIX)) {
	...
	concat(or_flags_, SUFFIX)( REG(0), imm);
	REG(0) |= imm;
	print_asm("or" " $0x%x,%%%s", imm, REG_NAME(0));
	return 1 + DATA_BYTE;
}

make_helper(concat(or_iv2rm_, SUFFIX)) {
	ModR_M m;
	m.val = instr_fetch(eip + 1, 1);
	DATA_TYPE imm;
	if(m.mod == 3) {
...
make_helper(concat(or_r2rm_, SUFFIX)) {
	...
	DATA_TYPE imm;
	if(m.mod == 3) {
		concat(or_flags_, SUFFIX)( REG(m.R_M), REG(m.reg));
		REG(m.R_M) |= REG(m.reg);
		...
		int len = read_ModR_M(eip + 1, &addr);
		imm = MEM_R(addr);
		concat(or_flags_, SUFFIX)( REG(m.reg), imm);
